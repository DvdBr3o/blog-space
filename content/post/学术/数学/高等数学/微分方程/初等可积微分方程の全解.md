...
lastmod: 2025-03-02T19:25:05+08:00
tags:
  - 高等数学
  - 微分
categories: 高等数学
...
这里介绍特殊情况下的**待定系数法**，更 general 的方法论见常数变易法
...
| $m$ 重共轭复根 | $(\lambda_{1})_{1,2}=\dots=(\lambda_{m})_{1,2}=\alpha\pm\beta i$ | $\begin{align}e^{ax}\cos\beta x,&e^{ax}\sin\beta x,\\ xe^{ax}\cos\beta x,&xe^{ax}\sin\beta x, \\ \dots,\\ x^{m-1}e^{ax}\cos\beta x,&x^{m-1}e^{ax}\sin\beta x\end{align}$ | $e^{ax}\sum_{i=1}^{m} x^{i-1}(C_{i,1}\cos\beta x+C_{i,2}\sin\beta x)$ |
